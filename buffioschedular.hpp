...
public:
  ...
  int processEvents(struct epoll_event evnts[], int len) {
    for (int i = 0; i < len; i++) {
        ...
        *handle | BUFFIO_READ_READY;
        if (req != nullptr) {
          if (evnts[i].events & EPOLLET) {
            ...
            buffioSockBroker::consumeEntry(req);
          };
        };
      }
      ...
      auto routine = buffioSockBroker::handleAsync((buffioHeader*)req);
      if (handle)
        queue.push(routine);
     };
    ...
    int consumeBatch(int cycle = 8) {
      ...
      for (int i = 0; i < cycle; i++) {
        ...
        int error = buffioSockBroker::consumeEntry(req);
        if (error == 1)
          goto continueloop;
        if (error <= 0) {
  ...
  private:
    /*
     * sycPipe is used to ping the event loop that the worker thread have
     * completed the work, and the eventloop can dequeue the qeueue and consume
     * the entry.
     *
     * The read side of the pipe is polled in event loop with header opCode
     * dequeueThread. and the write side of the pipe is sent to the worker
     * thread to write to ping the evloop.
     */
    buffioFd *syncPipe;
    /*
     * poller is the instance of sockbroker that contains the code for worker
     * and creation of queue, that the worker thread and the eventloop can use
     * to transfer task and receive task.
     *
     */
    buffioSockBroker poller;
    /*
     * timerClock is a class that implements all the time releated request
     * needed by the tasks,
     */
    ...
    /*
     * fdPool is the pool that have all tha fd that are currently being used and
     * every fd after being used, the fd must be returned to the pool, via the
     * appropriate call.
     */
    buffioFdPool fdPool;
    ...
    buffioQueue<> queue;
    /*
     * headerPool is the pool of header that the routine can used to get header,
     * to prevent from alocating new one, and after using a header it must be
     * returned to the headerpool, via appropriate call or method provided.
     *
     */
    buffioMemoryPool<buffioHeader> headerPool;
    /*
     * requestBatch is the circular linked list of requests that need
     * read/write, requestBatch is mostly used if edge-triggered is used in
     * epoll.
     */
     ...
     * but it doesn't manages the thread destruction, it must be managed by the
     * porgrammer.
     *
     */
