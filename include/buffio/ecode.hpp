...
constexpr int B_EUNKNOWN_OFFSET = 0x7fff;
...
#define BUFFIO_ERROR_TABLE(X)                                                  \
  ...
  X(B_EUNKNOWN, B_EUNKNOWN_OFFSET, "Unknown Error ")                                      \
  X(B_EABORT, (B_EUNKNOWN_OFFSET + 1), "Aborting loop/request")                         \
  X(B_ETHREAD, (B_EUNKNOWN_OFFSET + 2), "Error Creating thread(unknown)")               \
  X(B_ETHREADNOMEM, (B_EUNKNOWN_OFFSET + 3),                                            \
    "System out of resouce/max limit on number"                                \
    " of thread reached")                                                      \
  X(B_ETHREADARGS, (B_EUNKNOWN_OFFSET + 4),                                             \
    "Invalid args provided for thread creation")                               \
  X(B_ESEMCREATE, (B_EUNKNOWN_OFFSET + 5), "Error creating semaphore")                  \
  X(B_ELFORDER, (B_EUNKNOWN_OFFSET + 6), "lfQueue order not within limits")             \
  X(B_EINITTSKQUE, B_EUNKNOWN_OFFSET + 7, "Failed to init task queues in worker")     \
  X(B_EINITPOLLER, (B_EUNKNOWN_OFFSET + 8),                                             \
    "Failed to initlise the internal evloop poller")                           \
  X(B_EINITWRKTHR, (B_EUNKNOWN_OFFSET + 9), "Failed to create Worker thread")
...
//
// This is deliberately outside the buffio errno namespace.
// see X macro for buffio custom error code
