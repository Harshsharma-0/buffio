...
public:
  ...
  int create(size_t initialValue) {
    ...
    BUFFIO_WIN_INSERT(LONG lmaxCount = static_cast<LONG>(initialValue);
                      HANDLE semHandle =
                          CreateSemaphoreA(NULL, lmaxCount, lmaxCount, NULL);
                      lsem = semHandle; if (semHandle != INVALID_HANDLE_VALUE) return 0;)
