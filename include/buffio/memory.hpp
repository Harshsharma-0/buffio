 ...
 *
 * checksum is used to check, if the memory has maintained, it integrity after
 * use, and is usefull to find memorybugs for buffers, as if one buffer
 * overflows, it's invalidates the checksum of other memory region, corrupting
...
public:
   ...
   * @param[in] void(*deleter)(T *data) - custom deleter to call for all the
   * fagments in the page.
   *
   * @return 0 on success , -1 on error. Any value below 0 is treated as error.
   *
   */

  int init(size_t fragmentCount = 25, void (*deleter)(void *data) = nullptr) {
    chkSum = Memory::genChkSum(); // creating the chkSum for data
                                  // integrity;
   ...
   *   2. if there is no memory availabe, a new page is allocated and memory is
   * returned.
   *
   * @return pointer to the memoryfragment, or nullptr if there any error
   * occured.
   *
   */
   ...
   *
   * push() method accepts only the memory that was obtained from the memory
   * pool, any other memory from other sources(user allocated memory), causes
  ...
  /**
   * @brief method used to release all the pages and deallocate memory allocated
   * by the pool.
   *
   * @returns void
   */
