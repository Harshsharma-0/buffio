 ...
 * @note Before using these operation fd must be polled to get the defined behaviour:
 *  - waitRead/Write
 *  - asyncRead/write
 *  - asyncAccept/Connect
 *  - waitAccept/Connect
...
public:
 ...
 /**
   ...
   *
   * @note fd instance support only one subsiquent read/write request at any instance of time 
   * if there another request made while the pending one is not done, the request is dropped
   *
...
/**
   ...
   *
   * reserve header is internally used by buffioFd, for request other than read/write
   * 
   ...
   *
   * @param[in] addr pointer to sockaddr_(un/in/in6) to store the information of the accepted connection
   * @param[in] socklen pointer of the length of sockaddr_(un/in/in6)
   *
   * @return a valid fd reffering to the accepted connecton, and -1 if error occurs
   *
   * @note can pass NULL, if don't want to get the address information of the accepted socket
   */
   ...
   *
   * @param[in] addr pointer to struct sockaddr_(un/in/in6) if socket is not binded, or NULL
   * @param[in] socklen length of the struct sockaddr_(un/in/in6) if addr is provided, or can leave 0 or NULL
   *
   * @warning you must create the socket with the right address and portnumber you want to connect to,
   *    or provide addr and socklen to bind the socket to that address and port if the socket is not binded
   *
   ...
   * @param[in] header pointer to valid header that is obtained from the instance
   * @return void
   *
   */
   ...
   * @param[in] then valid buffioPromsieHandle to a handler to the socket after acception connection
   * @return void
   *
   * @note before asyncAccept, the user must call the poll method to register the socket for polling, and async accept must
   * be called with buffioawait 
   * ```cpp
   *  
   ...
   */
  template <typename T>
  buffioHeader *asyncAccept(T then) {
   ...
   * 
   * @param[in] addr pointer to sockaddr_(un/in/in6) to get the address of the accepted connection
   * @parma[in] socklen lenght of the respective sockaddr
   * @warning lifecycle of the sockaddr must be valid throughout the entire process of accept
   ...
   * @param[in] header pointer to valid buffioHeader that lifecycle must be valid for async read op
   * @param[in] buffer pointer to the buffer to read
   * @param[in] len size of the data in bytes to read
   *
   ...
   * @param[in] header pointer to valid buffioHeader that lifecycle must be valid for async write op
   * @param[in] buffer pointer to the buffer to write
   * @param[in] len size of the data in bytes to write
   *
   * @return buffioHeader crafted for asyncRead
...
private:
   ...
   * @param[in] address address of the fifo, must be a valid string allocated using new
   * @return void
   */
...
protected:
   ...
   * @note buffioHeader must be either allocated via the header pool or new operator
   *  it's a good practice to use buffiocall to get header memory so the memory leak can be prevented
   */
 ...
 *
 * @details buffioMakeFd is a helper wrapper to create socket, pipe,fifo or opening files easily,
 *  without dealing with low level stuff.
 *
...
public:
   ...
   * @param[out] lsocket pointer to struct sockaddr_(un/in/in6) based on the family of socket used
   * @param[in] address address of the socket to bind it to
   * @param[in] portNumber portnumber to bind the socket to, ignored for local socket
   * @param[in] family family of socket to create, see buffioFdFamily for list of supported family
   * @param[in] protocol protocol for the socket, see buffioSocketProtocol for list of supported protocol
   * @param[in] blocking set's the socket to blocking or non-blocking based on boolean value, if true socket is blocking
   *
   * @return returns buffioErrorCode, and any value below 0 must be treated as error and taken proper action
   *  on Success return buffioErrorCode::none that is 0.
   * @note The return value must not be discarded
   * @todo add generic type fd support, to suport returning fd via int not buffioFd so this class can be used by any program
   *
   */
  [[nodiscard]]
  static int
   ...
   * @param[in] blocking set's the socket to blocking and non-blocking mode based on boolean value
   *
   * @return returns buffioErrorCode::none on Success, onError return errorcode below 0
 ...
 *
 * @note the fd that is given back must originate from the pool or must be first pushed to the use queue of the pool,
 *       if these condition is not met, the pool becomes corrupt
 */
